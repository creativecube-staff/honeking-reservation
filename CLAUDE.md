...
- `shared/reservationPolicy.ts` - 予約運用ポリシー定数(`MAX_ADVANCE_DAYS=180`)。
...
### 1. 管理画面 UI を salonboard 風にリファクタリング

事業主からの要望で、現状の **WordPress 風管理画面**(黒紺サイドバー + 上部 admin bar)を **salonboard 風**(白〜薄グレーのヘッダー + 明るいサイドバー、本日の予約を主役にしたダッシュボード)に寄せる。

- 色は今のままオレンジ系で維持
- レイアウトと雰囲気のみ変更、機能は基本そのまま
- 着手前に salonboard のスクリーンショット入手 → 突き合わせて作業
- 関連: [[admin_ui_refactor_to_salonboard]] memory
