...
- 営業時間(両店共通):
  ...
  - **最終受付 = その日の最後のレンジの終了時刻**。お客様はその時刻まで予約開始でき、施術は閉店後にはみ出てよい(担当は開始時刻に出勤しているスタッフ。`shift.endMin >= 開始時刻` で判定し、施術終了がシフト終了を超えても可)。中抜け休憩前のレンジは従来どおり休憩前に施術が終わる枠のみ
- 定休日: **なし**(365日営業)
...
- `MANAGER`(店長) / `RECEPTIONIST` / `PRACTITIONER`: 所属店舗(`Practitioner.storeId`)のみ。スイッチャーは自店固定
...
- クライアント: `app/composables/useStoreContext.ts`(`useStoreContext()`)が選択中店舗を保持(cookie `admin_store`)
- セッション: ログイン時に `User.storeId` を保存(`server/api/admin/login.post.ts`)。DB スキーマ変更なし(既存 `Practitioner.storeId` を流用)
- **顧客台帳は全店共通**(誰でも検索可・スコープ対象外)。操作系(予約・売上・シフト等)のみ店舗で絞る
- 役職ごとの細かい権限差は permissions.ts が担当。店舗スコープとは別軸
- 横展開(予約一覧UI/シフト/スケジュール/売上/店舗設定)は Phase 2 で(下記「残作業」参照)
...
/dashboard/shifts         ← シフト管理(月/週/日/スタッフ別)
/dashboard/stores         ← 店舗管理(ベッド・特別メニュー・営業時間)
/dashboard/staff          ← スタッフ管理(ログイン情報・権限)
