...
- **店舗スコープ(アクセス境界)** — OWNER は全店、店長 / 受付 / 施術者は所属店舗のみ。スイッチャーで「管理者(全店) / 各店」を切替。操作系(予約・売上・スタッフ)は選択店舗に限定、顧客台帳は全店共通。**店舗切替時はダッシュボードに自動遷移**
- **予約・販売管理**
...
- **店舗管理** — 一覧(あいまい検索 + 都道府県 / 市区町村 / 表示順のソート + 新規追加)、詳細で基本情報・ベッド・営業時間を編集。**新規作成時にベッドと標準営業時間まで一括セットアップ**(作成直後から予約受付可能)。店休日は店舗スイッチャーで対象店に切り替えて登録
- **スタッフ管理** (店舗モード専用、名前・性別・役職・表示順・振り分け順・基本シフトをモーダルで編集、完全削除あり。出勤予定は基本シフト曜日 × 店舗営業時間で決まる、専用シフト管理画面は持たない)
- **ログイン管理** (全店モード・OWNER 専用、`/dashboard/accounts`。Login テーブルでパスワード自動生成・再発行・ログイン履歴を管理)
- **メニュー管理** — 共通メニュー(全店共通)/ 店舗特別メニューを**同一 UI で管理**(店舗スイッチャーで切替、あいまい検索・無効化/有効化/完全削除つき)
  - 共通メニュー → **店舗別非表示**: 「この店舗では出さない」のオプトアウト指定
  - 店舗特別メニュー → **共通メニューと差し替え**: アクティブな表示期間中はその共通メニューを自店で自動非表示にし、期間終了で自動復帰(キャンペーン用)
- **商品・売上管理**
...
| カレンダー(管理) | 自作 TimeColumnCalendar(縦軸=時刻、横軸=ベッド) |
...
- **Staff** — 店舗で働く人(施術者・受付・店長)。`name` / `gender` / `role`(表示用) / `displayOrder` / `assignOrder` / `baseShiftDays`(出勤曜日 Int[]) / `isActive` / `isAssignable` を持つ。出勤予定はこの基本シフトと店舗 BusinessHour の組み合わせで決まる
- **Login** — 管理画面のログインアカウント(Staff とは完全独立)。`username` / `passwordHash` / `passwordEnc`(AES-256-GCM・OWNER が後で確認可) / `role` / `permissions` / `totpSecret` / `lastLoginAt`
- **LoginHistory** — 認証履歴(成功/失敗 + IP + UA、`onDelete: SetNull`)
...
- **Reservation** — 予約(Store + Bed + Staff + Menu + 時間帯) + **ReservationHistory** — 変更履歴(操作者は `changedByLoginId`)
- **Product** / **ProductSale** — 商品マスタ + 販売記録(物販・回数券、`soldByLoginId` で販売登録者を記録)
- **CustomerVoucher** / **VoucherUsage** — 回数券保有・消費

予約の重複は **PostgreSQL `EXCLUDE USING gist` 制約** で DB レベル防止(同じベッド・スタッフの時間重複を不可能にする)。
