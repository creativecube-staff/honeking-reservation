...
- **日時選択** — 時刻×曜日グリッドで `◯/△/要TEL` を表示、`要TEL` は SP で `tel:` 起動
...
│   ├─ components/
│   │   ├─ Auth/AuthModal.vue             # 予約フロー中の in-page ログイン/新規登録モーダル
...
│   │   ├─ reservations.post.ts           # 予約作成(完了メール送信込み)
...
### Phase 5.6: 予約フロー SPA 化 + UI 統一

- お客様側予約フローを **SPA 化**(`/[slug]` 一枚にメニュー/日時/確認を内部ステートで集約、LIFF 組み込み想定)
- 共通 **ReservationStepIndicator** コンポーネント(クリック可能な done ステップ、円下の選択値タグ、進捗ラインのフィルアニメ)
- 予約フロー中の **in-page 認証モーダル** (`AuthModal`):別ページに飛ばずにログイン/新規登録、SPA ステートが保持される
- メニューカードの 3 カラムグリッド + 期間限定バッジ + 価格強調
- メニュー・日時・確認ステップで統一的なストリップ表示(店舗 / メニュー / 日時)
- マイページ・プロフィール変更等のフォームページを `max-w-5xl` 外枠 + `max-w-lg` 中央寄せフォームに統一
- 予約完了メール(Resend、ロゴ + 店舗写真入り HTML)を実装
- ファビコン・apple-touch-icon は honeking.jp 本体と共有

### Phase 6: 本番デプロイ

VPS (または Oracle Cloud Always Free) + Supabase Pro 昇格 + Cloudflare R2 バックアップ + HTTPS + 2FA(TOTP)。本番 Caddyfile は dev の `Caddyfile` を雛形に Let's Encrypt 有効化 + HSTS/CSP 追加 + IP allowlist + ホスト名差し替え。
...
### Phase 7(検討中): 外部予約サイト取込

EPARK / HotPepper 等の外部予約サイトからの予約通知メールを Cloudflare Email Workers 経由でパースし、自動で予約登録。失敗時は管理画面の「未取込メール」一覧 + Slack 通知でフォールバック。

### Phase 8(検討中): LIFF (LINE) 統合

- LINE Developers で Provider + Channel + LIFF アプリ作成
- `@line/liff` SDK 組み込み、`Customer.lineUserId` で LINE userId と顧客を紐付け
- LIFF 経由アクセス時はプロフィール自動取得 → 予約フォームに自動入力
- LINE Messaging API で予約完了通知・前日リマインダーを push
- 公式アカウントのチャット機能とは独立して共存(ハイブリッドモード)
